import os, asyncio, base64, uuid, traceback
...
async def create_task(body: dict):
    ...
    if task_type in ("RecaptchaV3TaskProxyless", "RecaptchaV3TaskProxylessM1",
                     "RecaptchaV3TaskProxylessM1S7", "RecaptchaV3TaskProxylessM1S9",
                     "RecaptchaV3EnterpriseTask", "RecaptchaV3EnterpriseTaskM1",
    ...
    return {"errorId": 1, "errorCode": "ERROR_UNKNOWN_TASK_TYPE", "errorDescription": f"Unsupported: {task_type}"}
...
async def handle_image_task(task):
    ...
    try:
        async with httpx.AsyncClient(timeout=120) as client:
            r = await client.post(f"{SHIM_URL}/solve-json", json={
                "image": image_b64,
                "instruction": instruction
            })
            data = r.json()
            if data.get("success"):
                return {"errorId": 0, "status": "ready", "solution": {"text": data.get("answer", "")}}
            return {"errorId": 1, "errorCode": "ERROR_SHIM_FAILED", "errorDescription": data.get("error", "Shim error")}
...
async def handle_recaptcha_v2_image(task):
    ...
    if not url:
        return {"errorId": 1, "errorCode": "ERROR_BAD_PARAMETERS", "errorDescription": "Missing URL"}
    ...
    try:
        async with async_playwright() as p:
            ...
            try:
                checkbox = page.frame_locator('iframe[src*="recaptcha/api2/anchor"]').locator('.recaptcha-checkbox-border')
            ...
            # Wait for image challenge iframe
            try:
                ...
                token = await _wait_for_recaptcha_token(page)
                await context.close()
                if token:
                    return {"errorId": 0, "status": "ready", "solution": {"gRecaptchaResponse": token}}
                return {"errorId": 1, "errorCode": "ERROR_TOKEN_NOT_FOUND", "errorDescription": "No image challenge and no token"}

            # Get instruction text
            try:
                instruction_el = challenge_frame.locator('.rc-imageselect-instructions')
                instruction_text = await instruction_el.inner_text()
                instruction = f"{instruction_text.strip()}. Return ONLY comma-separated tile numbers like 1,3,6."
            except:
                instruction = "Select all images containing the requested object. Return ONLY comma-separated tile numbers like 1,3,6."
            ...
            async with httpx.AsyncClient(timeout=120) as client:
                ...
                if not data.get("success"):
                    await context.close()
                    return {"errorId": 1, "errorCode": "ERROR_VISION_FAILED", "errorDescription": data.get("error", "Vision failed")}
            ...
            if token:
                return {"errorId": 0, "status": "ready", "solution": {"gRecaptchaResponse": token}}
            return {"errorId": 1, "errorCode": "ERROR_TOKEN_NOT_FOUND", "errorDescription": "Image grid solved but token not generated"}

    except Exception as e:
        traceback.print_exc()
        return {"errorId": 1, "errorCode": "ERROR_CDP_FAILED", "errorDescription": str(e)[:200]}
...
async def solve_token_task_async(task_id, task_type, task):
    ...
    if not url or not sitekey:
        tasks[task_id] = {"errorId": 1, "errorCode": "ERROR_BAD_PARAMETERS", "errorDescription": "Missing URL or sitekey"}
        return
            ...
            if "RecaptchaV3" in task_type:
                token = await page.evaluate(f"""
                    new Promise((resolve, reject) => {{
                        ...
                        grecaptcha.ready(() => {{
                            grecaptcha.execute('{sitekey}', {{action: '{task.get("pageAction", "verify")}'}}).then(resolve).catch(reject);
                        }});
                    }}
                """, timeout=30000)
                tasks[task_id] = {"errorId": 0, "solution": {"gRecaptchaResponse": token}}
                ...
                if token:
                    ...
                    tasks[task_id] = {"errorId": 1, "errorCode": "ERROR_TOKEN_NOT_FOUND", "errorDescription": "reCAPTCHA v2 token not found"}

            elif "HCaptcha" in task_type:
                try:
                    await page.frame_locator('iframe[src*="hcaptcha"]').locator('#checkbox').click(timeout=5000)
                except:
                    pass
                ...
                if token:
                    ...
                    tasks[task_id] = {"errorId": 1, "errorCode": "ERROR_TOKEN_NOT_FOUND", "errorDescription": "hCaptcha token not found"}
                ...
                if token:
                    ...
                    tasks[task_id] = {"errorId": 1, "errorCode": "ERROR_TOKEN_NOT_FOUND", "errorDescription": "Turnstile token not found"}

            else:
                tasks[task_id] = {"errorId": 1, "errorCode": "ERROR_UNKNOWN_TASK_TYPE", "errorDescription": f"Token type not implemented: {task_type}"}

            await context.close()

    except Exception as e:
        traceback.print_exc()
        tasks[task_id] = {"errorId": 1, "errorCode": "ERROR_CDP_FAILED", "errorDescription": str(e)[:200]}
